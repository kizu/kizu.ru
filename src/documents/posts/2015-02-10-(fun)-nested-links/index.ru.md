...
Если прописать в атрибут `type` неизвестный природе MIME-тип, то браузер сразу же перейдёт к отображению фолбека. Но он [это сделает](*update "На самом деле, см. [дополнение к статье](#update-from-2015-03-05)") и в том случае, если мы вообще не зададим ни один из «обязательных» атрибутов.
